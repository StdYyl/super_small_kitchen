  ...
  <div>
    ...
    <el-form ref="form" :model="registerForm" label-width="160px" class="fm" :rules="rules">
      ...
      <el-form-item>
        ...
        <el-button @click="previous">{{indexed.length == 1?'放回':'上一步'}}</el-button>
      </el-form-item>
    </el-form>
  </div>
  ...
  methods: {
    next() {
      this.$refs.form.validate((valid) => {
        if (valid) {
          // 切换问题：this.indexed赋值 子组件的值不会改变  this.indexed[1] = 1;
          if (this.indexed.length == 5) {
            // let warn = document.getElementsByClassName("el-message--warning");
            // for(let i = 0 ; i < warn.length ; i++){
            //     console.log(warn[i].style.top)
            //     warn[i].style.top = "20px"
            // }
            ...
            // console.log(warn.style.top)
            // warn.setStyle.top = "20px"
    ...
    previous() {
      if (this.indexed.length == 1) return;
